...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('system',['upgradeSystem','getUpgradeStatus']),
      handleBeforeUpload(){
        return false
      },
      handleChange(info) {
      ...
      startUpload(){
        ...
        const params = {firmware: this.file}
        this.upgradeSystem(params).then((res) => {
          if (res.status === 200){
            this.timerGetText()
          }else {
            this.upgradeFail()
      ...
      timerGetText(){
        this.timer = setTimeout( () => {
          this.getUpgradeStatus().then((res) => {
            if (res.status === 200){
              let temp = res.data.trim().split('\n')
              this.progress.text = temp[temp.length - 1]
              if (this.progress.text.includes('Sucessed.Rebooting')){
